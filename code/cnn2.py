...
def load_data():
	...
	xtest = '../Data/test_x.bin'
        x = numpy.fromfile(xtrain, dtype='uint8')
        x = x.reshape((100000,60,60))
	test = numpy.fromfile(xtest, dtype='uint8')
...
model.add(Convolution2D(120, 7, 7, border_mode='valid',input_shape=X_train.shape[1:],activation='relu'))

model.add(MaxPooling2D(pool_size=(3, 3)))
model.add(Convolution2D(120, 7, 7,activation='relu'))
model.add(MaxPooling2D(pool_size=(3, 3)))
model.add(Convolution2D(120, 7, 7,activation='relu'))
model.add(MaxPooling2D(pool_size=(3, 3)))
...
if not data_augmentation:
    ...
    preds.to_csv('../Data/probability.csv')
